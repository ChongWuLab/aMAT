...
for (i in 1:n.perm) {
    ...
    res[1,1] = res[1,1] + proc.time()[3] - start.time

    start.time = proc.time()[3]
    pSSU <- SumSqU_fast(tmp.z, beta1, alpha1, d1)
    res[2,1] = res[2,1] + proc.time()[3] - start.time
    
    start.time = proc.time()[3]
    #pATrev <- MultiXcan_fast(tmp.z, t.matallrev, multi.dfallrev)$multi.p
    pATall <- MultiXcan_fast(tmp.z, t.matall, multi.dfall)$multi.p
    res[3,1] = res[3,1] + proc.time()[3] - start.time
    ...
    res[4,1] = res[4,1] + proc.time()[3] - start.time
    ...
    pAT30 <- MultiXcan_fast(tmp.z, t.mat30, multi.df30)$multi.p
    pAT50 <- MultiXcan_fast(tmp.z, t.mat50, multi.df50)$multi.p
    res[5,1] = res[5,1] + proc.time()[3] - start.time
     ...
     res[6,1] = res[6,1] + proc.time()[3] - start.time


}

res[7,1] = time1
rownames(res)= c("SUM","SSU","chi-square","SHom","MAT","aMAT","estimating")
save(res,n.perm, file=paste("/gpfs/research/chongwu/Chong/Multitrait-snp/simulation/res/runing_time/",idpgroup,"_",job.id, ".RData", sep = ""))
